...
- **Re-OCR & upload library:** `GET /api/ocr/jobs` lists past uploads (the "Thư viện" page, `pages/Uploads.tsx`; receptionist/nhân viên sees own, others see all). `POST /api/ocr/jobs/{id}/reocr` re-queues an existing image, optionally with a new `jobs.rotate` (0/90/180/270) — used from the Review page's "OCR lại" modal since wrong rotation is the main cause of bad reads. Per-job `rotate` overrides `FINOS_OCR_ROTATE`. Both `stage` and `rotate` columns are added idempotently in `database.py` (ALTER TABLE migration).
- **OCR = a VLM via Ollama** (`backend/app/ocr/`): `vlm.py` sends the **original full-resolution image** to the VLM for integrity — no downscale. It only rotates upright when `FINOS_OCR_ROTATE != 0` (default 90° — ledgers are photographed sideways; a 90° rotate is a lossless pixel permutation, re-encoded JPEG q95/4:4:4). With `FINOS_OCR_ROTATE=0` the raw file bytes are sent untouched. It base64-encodes and POSTs (streaming) to Ollama's `/api/generate` (`gemma4:31b-cloud`, `format:"json"`, `temperature:0`) with a prompt that asks for `{phong, tien}` per row. The client uploads the original photo too (no compression). `pipeline.py` turns each item into an `OcrRow`-shaped record via `_ledger_amount` (the "nghìn" convention: bare `60`→60.000, `180`→180.000; a thousands-separated `1.200.000` is kept as-is). **The whole detect-box pipeline (RapidOCR/OpenCV) was removed** — it couldn't read handwriting or circled totals. `parse.py` remains only for the manual-entry/smoke-test path.
...
- **Auth/RBAC** (`security.py`, `deps.py`): JWT (HS256, PyJWT) + Argon2 password hashing. `get_current_user` resolves the bearer token; `require_roles(...)` builds per-route role guards. Roles: `admin` (everything + user mgmt + reports + activity log), `accountant` (enter/approve/edit + reports; transaction delete is soft delete), `receptionist`/nhân viên (capture/OCR + create/view/edit transactions, no delete, dashboard summary is clamped to today, and can only access own OCR jobs). Stats timeseries remains guarded by `require_roles("admin","accountant")`; summary is available to all roles with receptionist clamped to today.

- **Monthly Excel reports** (`reports.py`, `routers/reports.py`, `jobs/report_scheduler.py`): a `reports` table holds one row per month (`period` 'YYYY-MM', UNIQUE). `reports.py` builds a styled `.xlsx` (openpyxl — pure-Python, no native deps) with two sheets (*Tổng hợp* = KPI + daily breakdown, *Chi tiết* = all transactions) by aggregating the `transactions` table (approved data only — reports never touch the OCR flow). Files go to `FINOS_REPORT_DIR`; regenerating a month overwrites the file + upserts the row (`ON CONFLICT(period)`). `ReportScheduler` is a background thread (mirrors `push_worker`/`OcrWorker`) that, when `FINOS_AUTO_MONTHLY_REPORT=true`, auto-renders the just-completed month on startup/poll and notifies admins (`report.ready`). Endpoints (`/api/reports`): list / generate-now / download / delete — view+generate are `admin`+`accountant`, delete is `admin` only. The `Reports.tsx` page (nav "Báo cáo") lists reports, has a month-picker "Tạo báo cáo" button, and downloads the xlsx via an authed blob fetch in `api.ts`.
...
All settings use the `FINOS_` env prefix (read from `backend/.env` via pydantic-settings; see `config.py`). Key ones: `FINOS_SECRET_KEY`, `FINOS_ADMIN_PASSWORD` (seeds the first admin), `FINOS_DB_PATH`, `FINOS_UPLOAD_DIR`, `FINOS_MAX_UPLOAD_MB`. OCR/VLM: `FINOS_OLLAMA_HOST` (e.g. `http://localhost:11434`, or `http://host.docker.internal:11434` from Docker), `FINOS_OCR_MODEL` (`gemma4:31b-cloud`), `FINOS_OCR_ROTATE` (90; 0 = send raw original), `FINOS_OCR_TIMEOUT_SECONDS`, `FINOS_MAX_UPLOAD_MB` (25 — originals are uncompressed). Reports: `FINOS_REPORT_DIR` (where `.xlsx` files are stored — point at the persistent volume in Docker, e.g. `/data/reports`), `FINOS_HOTEL_NAME` (printed on the report header), `FINOS_AUTO_MONTHLY_REPORT` (default true; set false to disable the auto month-end scheduler). **Change `FINOS_SECRET_KEY` and the admin password before any real use.**

Running OCR requires Ollama up with the model pulled: `ollama pull gemma4:31b-cloud`. Without it, uploads queue and jobs fail. `poc/ocr_vlm_test.py <image>` is the standalone tester (supports `--rotate`, `--max-side`, `--raw`).
