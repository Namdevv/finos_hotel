    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <p className="text-sm text-slate-500">
          {employeeView ? "Tổng thu / chi hôm nay" : group === "day" ? "Số liệu tháng này" : "Số liệu theo tháng"}
        </p>
        {!employeeView && (
          <select
            value={group}
      ...
      {!employeeView && (
        <Card>
          <div className="mb-4 flex items-center justify-between">
            ...
            {summary && <span className="text-xs text-slate-400">{summary.count} chứng từ</span>}
          </div>
          {!series ? (
            <Spinner />
          ) : series.length === 0 ? (
