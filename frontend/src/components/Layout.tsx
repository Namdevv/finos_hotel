...
import { useSystemNotifications } from "../notify";
...
// "Người dùng" đã được chuyển vào trang Hồ sơ (profile setting) — không còn ở
// sidebar nữa. Admin truy cập quản lý người dùng qua menu trong trang Hồ sơ.
const NAV: NavItem[] = [
...
export default function Layout() {
  ...
  // Đẩy thông báo (vd OCR xong) ra thông báo hệ thống khi tab đang ẩn.
  ...
  return (
    <div className="flex min-h-screen bg-slate-100">
      <aside className="fixed inset-y-0 left-0 hidden w-64 flex-col bg-ink-900 text-slate-300 md:flex">
        <Link to="/" className="flex items-center gap-3 px-5 py-5" title="Về trang Tổng quan">
        ...
        <header className="sticky top-0 z-20 flex h-14 items-center justify-between border-b border-slate-200 bg-white/90 px-4 backdrop-blur md:px-8">
          <div className="flex min-w-0 items-center gap-2">
            <Link to="/" aria-label="Về trang Tổng quan" className="shrink-0 md:hidden">
              <img src="/logo_finos.png" alt="FinOS Hotel" className="h-7 w-7 rounded-md object-cover" />
            </Link>
          ...
          <div className="flex shrink-0 items-center gap-2 sm:gap-3">
            ...
            <button
              onClick={() => nav("/profile")}
              title="Hồ sơ người dùng"
              className="flex h-8 w-8 cursor-pointer items-center justify-center rounded-full bg-brand-100 text-sm font-bold text-brand-700 transition-colors hover:bg-brand-200"
            >
              {initials}
            </button>
          </div>
